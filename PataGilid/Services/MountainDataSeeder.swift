...
class MountainDataSeeder {
    ...
    var officialMountains: [Mountain] {
        guard let url = Bundle.main.url(forResource: "philippine_mountains", withExtension: "json"),
              ...
              let mountains = try? JSONDecoder().decode([Mountain].self, from: data) else {
            print("⚠️ Failed to load bundled philippine_mountains.json resource.")
            return []
        }
        return mountains
    }
    ...
    func seedMountainsIfNeeded() async throws {
        ...
        for chunk in mountains.chunked(into: chunkSize) {
            ...
            for mountain in chunk {
                guard let documentId = mountain.id else { continue }
                let docRef = collection.document(documentId)
                try batch.setData(from: mountain, merge: true)
            }
